...
static char *libraryInfo      = "P.E.Op.S. SoftX Driver V1.17\nCoded by Pete Bernert and the P.E.Op.S. team\n";
#else
static char *libraryName      = "P.E.Op.S. SoftSDL Driver";
static char *libraryInfo      = "P.E.Op.S. SoftSDL Driver V1.17\nCoded by Pete Bernert and the P.E.Op.S. team\n";
#endif
#endif
...
unsigned char __attribute__((aligned(8))) *psxVSecure;
unsigned char __attribute__((aligned(8))) *psxVub;
signed   char __attribute__((aligned(8))) *psxVsb;
unsigned short __attribute__((aligned(16))) *psxVuw;
unsigned short __attribute__((aligned(16))) *psxVuw_eom;
signed   short __attribute__((aligned(16))) *psxVsw;
unsigned long  __attribute__((aligned(16))) *psxVul;
signed   long  __attribute__((aligned(16))) *psxVsl;
...
long CALLBACK GPUinit()                                // GPU INIT
{
 ...
 //memcpy(psxVuw,(unsigned short*)psxVub,sizeof(psxVub));
...
//	pspDebugScreenPrintf("  Yoshihiro present PSXP based on PCSX CORE 1.6 Beta\n ");
//	pspDebugScreenPrintf(" PSXP Alpha 2  %s FPS on 60  Resolution %d x %d %s \n",szDispBuf,PSXDisplay.DisplayModeNew.x,PSXDisplay.DisplayModeNew.y, PSXDisplay.PAL ? "PAL 50Hz" : "NTSC 60Hz" );
 if(iFastFwd)                                          // fastfwd ?
 {
...
//       if(VRAMWrite.ImagePtr>=psxVuw_eom) VRAMWrite.ImagePtr-=iGPUHeight*1024;  // smhzc switch around
  //     *VRAMWrite.ImagePtr++ = (unsigned short)gdata; // smhzc switch around
     ...
     //  if(VRAMWrite.ImagePtr>=psxVuw_eom) VRAMWrite.ImagePtr-=iGPUHeight*1024; // smhzc
	//	*VRAMWrite.ImagePtr++ = (unsigned short)(gdata>>16); // smhzc 
 ...
 if(DataWriteMode==DR_NORMAL)
  {
   ...
   for(;i<iSize;)
    {
	   if(DataWriteMode==DR_VRAMTRANSFER){
		//	DLog("back to start vram");
		 goto STARTVRAM;
	   }else{
	   	//	DLog("continuing");
	   }
	//DLog("writing to pMem %d",pMem);
     gdata=*pMem++;
	 i++;
	//DLog("after write");
     if(gpuDataC == 0)
...
//if(command>=0xb0 && command<0xc0) auxprintf("b0 %x!!!!!!!!!\n",command);
	...
	//DLog("after write 2");
     ...
     if(gpuDataP == gpuDataC)
      {
       gpuDataC=gpuDataP=0;
	  // if(gpuDataM >= psxVuw_eom) return;
	   //DLog("calling func %d on data %d",gpuCommand,gpuDataM);
	   //if(gpuCommand== 124)continue; // smhzc
	   //issue with 44 as well
       primFunc[gpuCommand]((unsigned char *)gpuDataM);
	   //DLog("after calling func");
      }
    } 
  }
...
//DLog("GPUdmaChain2");
   ...
   dmaMem=addr+4;
//DLog("GPUdmaChain3 %d %d",count,dmaMem);
 //	if(count==9)break; //horrible horrible hack by steveh smhzc
