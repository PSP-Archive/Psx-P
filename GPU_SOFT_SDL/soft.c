/***************************************************************************
                          soft.c  -  For RISC CPU PORT By Yoshihiro 
						  description
                             -------------------
    begin                : Sun Oct 28 2001 by peter bernet 
	finish               : thu Oct 31 2006 by Yoshihiro 
...
// History of changes:
//
// 2006/10/31 - Yoshihiro 
// - added Fixed for CPU RISC 32BIT fixw all ptr to 2 ptr 16BIT 
//
// 2005/06/11 - Pete
   ...
   for(j=xmin;j<xmax;j+=2) 
    {
     //GetShadeTransCol32((unsigned long *)&psxVuw[(i<<10)+j],lcolor);
	   ...
	   GetShadeTransCol(&psxVuw[(i<<10)+j+1],color);
    }
   if(j==xmax)
    GetShadeTransCol(&psxVuw[(i<<10)+j],color);
     ...
     if(xmax>=xmin)
      {
       ...
       for(j=xmin;j<xmax;j+=2)
		...
		GetTextureTransColG_S(&psxVuw[(i<<10)+j+1],psxVuw[clutP+tC2]);
         posX+=difX2;
         posY+=difY2;
        }
       if(j==xmax)
     ...
     for(j=xmin;j<xmax;j+=2)
      {
       ...
       tC2=(tC2>>((XAdjust&1)<<2))&0xf;
		...
		GetTextureTransColG(&psxVuw[(i<<10)+j+1],psxVuw[clutP+tC2]); 
     ...
     if(xmax>=xmin)
      {
       ...
       for(j=xmin;j<xmax;j+=2)
		...
		GetTextureTransColG_S(&psxVuw[(i<<10)+j+1],psxVuw[clutP+tC1]);
		GetTextureTransColG_S(&psxVuw[(i<<10)+j],psxVuw[clutP+tC2]);
     ...
     for(j=xmin;j<xmax;j+=2)
      {
       ...
       tC2=(tC2>>((XAdjust&1)<<2))&0xf;
		...
		GetTextureTransColG_SPR(&psxVuw[(i<<10)+j+1],psxVuw[clutP+tC2]); 
     ...
     if(xmax>=xmin)
      {
       ...
       for(j=xmin;j<xmax;j+=2)
        {
 		GetTextureTransColG_S(&psxVuw[(i<<10)+j+1],((psxVuw[((((posY+difY)>>16)+GlobalTextAddrY)<<10)+((posX+difX)>>16)+GlobalTextAddrX])<<16)); 
		GetTextureTransColG_S(&psxVuw[(i<<10)+j],(psxVuw[(((posY>>16)+GlobalTextAddrY)<<10)+((posX)>>16)+GlobalTextAddrX])); 
   ...
   if(xmax>=xmin)
    {
     ...
     for(j=xmin;j<xmax;j+=2)
      {
       GetTextureTransColG(&psxVuw[(i<<10)+j],psxVuw[((((posY+difY)>>16)+GlobalTextAddrY)<<10)+((posX+difX)>>16)+GlobalTextAddrX]);
       GetTextureTransColG(&psxVuw[(i<<10)+j],psxVuw[(((posY>>16)+GlobalTextAddrY)<<10)+((posX)>>16)+GlobalTextAddrX]);
     ...
     if(xmax>=xmin)
      {
       ...
       for(j=xmin;j<xmax;j+=2)
        {
         GetTextureTransColG_S(&psxVuw[(i<<10)+j+1],
   ...
   if(xmax>=xmin)
    {
     ...
     for(j=xmin;j<xmax;j+=2)
      ...
      GetTextureTransColG(&psxVuw[(i<<10)+j],
         psxVuw[(((posY>>16)+GlobalTextAddrY)<<10)+(posX>>16)+GlobalTextAddrX]);
      GetTextureTransColG(&psxVuw[(i<<10)+j+1],
         psxVuw[(((posY>>16)+GlobalTextAddrY)<<10)+((posX)>>16)+GlobalTextAddrX]);
     ...
     if(xmax>=xmin)
      {
       ...
       for(j=xmin;j<xmax;j+=2) 
        ...
        psxVuw[(i<<10)+j]=(((cR1) >> 9)&0x7c00)|(((cG1) >> 14)&0x03e0)|(((cB1) >> 19)&0x001f)|lSetMask;
   ...
   if(xmax>=xmin)
    {
     ...
     for(j=xmin;j<=xmax;j++) 
      {
		 GetShadeTransCol(&psxVuw[(i<<10)+j+1],((cR1 >> 9)&0x7c00)|((cG1 >> 14)&0x03e0)|((cB1 >> 19)&0x001f));
       ...
       for(j=xmin;j<xmax;j+=2) 
        {
         ...
         GetTextureTransColGX_S(&psxVuw[(i<<10)+j+1], 
              psxVuw[clutP+tC1],
              (cB1>>16),(cG1>>16),(cR1>>16));
         GetTextureTransColGX_S(&psxVuw[(i<<10)+j], 
              psxVuw[clutP+tC1],
              ((cB1+difB)&0xff0000),((cG1+difG)&0xff0000),((cR1+difR)&0xff0000));
     ...
     for (sprCX=0;sprCX<sprtW;sprCX++)
      {
	   ...
	   //FIXE +1 for the first ptr 
       GetTextureTransColG_SPR(&psxVuw[sprA+1],psxVuw[clutP+((tC>>4)&0xf)]);
       GetTextureTransColG_SPR(&psxVuw[sprA],psxVuw[clutP+(tC&0xf)]);
      }
    return;
...
//32Bit Fixe Yoshihiro now sent 16Bit data in 2 times 
 else{ DrawSoftwareSpriteMirror(baseAddr,w,h);return;}
...
//   else return;
 ...
 //       GetTextureTransColG32_SPR((unsigned long *)&psxVuw[sprA],
   //         (((long)psxVuw[clutP+tC2])<<16)|
     //       psxVuw[clutP+tC]);
        GetTextureTransColG_SPR((unsigned short *)&psxVuw[sprA+1],
            ((psxVuw[clutP+tC2])<<16)|
            psxVuw[clutP+tC]);
        GetTextureTransColG_SPR((unsigned short *)&psxVuw[sprA],
            ((psxVuw[clutP+tC2])<<16)|
            psxVuw[clutP+tC]);       
    ...
    for (sprCY=0;sprCY<sprtH;sprCY++)
     {
      ...
      for (sprCX=0;sprCX<sprtW;sprCX+=2,sprA+=2)
       { 
    //    GetTextureTransColG32_SPR((unsigned short *)&psxVuw[sprA],
      //      (((long)psxVuw[(sprCY<<10) + textX0 + sprCX +1])<<16)|
        //    psxVuw[(sprCY<<10) + textX0 + sprCX]);
	GetTextureTransColG_SPR((unsigned long *)&psxVuw[sprA+1],
            ((psxVuw[(sprCY<<10) + textX0 + sprCX +1])<<16)|
            psxVuw[(sprCY<<10) + textX0 + sprCX]);
	GetTextureTransColG_SPR((unsigned short *)&psxVuw[sprA],
            ((psxVuw[(sprCY<<10) + textX0 + sprCX +1])<<16)|
            psxVuw[(sprCY<<10) + textX0 + sprCX]);
