...
{
 /*f (QueryPerformanceFrequency (&CPUFrequency))        // timer mode
...
unsigned long faketime;
unsigned long curr_tick;
unsigned long timeGetTime() // smhzc disable
{
 struct timeval tv;
...
//Good result wierd but good Yoshihiro ^_^
//return curr_tick/10;
}
