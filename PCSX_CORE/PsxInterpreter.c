...
// Interpreter Fixed for psp by yoshihihro ^_^ Good result at Toshinden texture it's back like under windows 
// Fixed psx Cycle
...
void psxDIV() {
	if (_i32(_rRt_) != 0) {
		psxRegs.GPR.n.lo = _i32(_rRs_) / _i32(_rRt_);
		psxRegs.GPR.n.hi = _i32(_rRs_) % _i32(_rRt_);
	}
}
...
/// FPSE Mult / multu
static void multu(unsigned long v1,unsigned long v2)
{
	unsigned long	v1h,v1l,v2h,v2l;
	unsigned long	ll,hl,lh,hh,mid,mid2;

	v1l = (unsigned short)v1;
	v1h = ((unsigned long)v1)>>16;
	v2l = (unsigned short)v2;
	v2h = ((unsigned long)v2)>>16;
	...
	psxRegs.GPR.n.lo = (unsigned short)ll|(mid2<<16);
	psxRegs.GPR.n.hi = hh+(mid>>16);
}

static void mult(long v1,long v2)
{
	int sign = (v1^v2)<0;
	...
	multu(v1,v2);
	if (sign) {
		psxRegs.GPR.n.lo = -psxRegs.GPR.n.lo;
		psxRegs.GPR.n.hi = ~psxRegs.GPR.n.hi;
		if (psxRegs.GPR.n.lo==0) psxRegs.GPR.n.hi++;
	}
}
