...
// - adjusted SPU_d_ irqs again (Thousant Arms, Valkyrie Profile)
...
//
// 2003/02/19 - kode54
// - moved SPU_d_ IRQ handler and changed sample flag processing
//
// 2003/02/18 - kode54
...
//
// 2002/12/28 - Pete
// - adjusted SPU_d_ irq handling, fmod handling and loop handling
//
// 2002/08/14 - Pete
// - added extra reverb
//
// 2002/06/08 - linuzappz
// - SPU_d_update changed for SPU_d_async
//
// 2002/05/15 - Pete
...
unsigned long   SPUAddr=0xffffffff;                    // address into SPU_d_ mem
...
// 2003/06/07 - Pete
#ifndef NOTHREADLIB
static int thread = -1;                          // thread id (linux)
#endif
#endif

unsigned long dwNewChannel=0;                          // flags for faster testing, if new channel starts

void (CALLBACK *irqCallback)(void)=0;                  // func of main emu, called on SPU_d_ irq
...
static int lastch=-1;      // last channel processed on SPU_d_ irq in timer mode
 ...
 while(!bEndThread)                                    // until we are shutting down
  {
   ...
   //  Sleep(PAUSE_L);                                  // else sleep for x ms (linux)
#endif
           ...
           if(pChannel->iSBPos==28)                    // 28 reached?
            {
             ...
             //////////////////////////////////////////// SPU_d_ irq handler here? mmm... do it later
...
ENDX:   ;                                                      
  ...
  // we know: 
  // the decode buffers are located in SPU_d_ memory in the following way:
  ...
  // Therefore: we just need a pointer from SPU_d_mem+0 to SPU_d_mem+3ff, and 
  // increase this pointer on each sample by 2 bytes. If this pointer
  // (or 0x400 offsets of this pointer) hits the SPU_d_irq address, we generate
...
////////////////////////////////////////////////////////////////////////
// SPU_d_ ASYNC... even newer epsxe func
...
void CALLBACK SPU_d_async(unsigned long cycle)
{
 ...
 if(iUseTimer==2)                                      // special mode, only used in Linux by this SPU_d_ (or if you enable the experimental Windows mode)
  {
...
// since epsxe 1.5.2 (linux) uses SPU_d_update, not SPU_d_async, I will
// leave that func in the linux port, until epsxe linux is using
// the async function as well

void CALLBACK SPU_d_update(void)
{
 SPU_d_async(0);
}
...
////////////////////////////////////////////////////////////////////////
// SPU_d_INIT: this func will be called first by the main emu
////////////////////////////////////////////////////////////////////////
...
/* if(!iUseTimer)                                        // linux tread?
  {
   int i=0;
   while(!bThreadEnded && i<2000) {Sleep(1000L);i++;} // -> wait until thread has ended
 ...
 bThreadEnded=0;                                       // no more SPU_d_ is running
 bSpuInit=0;
}

////////////////////////////////////////////////////////////////////////
// SETUPSTREAMS: init most of the SPU_d_ buffers
////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////
// SPU_d_OPEN: called by main emu after init
////////////////////////////////////////////////////////////////////////
...
#ifndef _WINDOWS
void SPU_d_setConfigFile(char * pCfg)
{
//SoundConfigFile=pCfg;
}
#endif

////////////////////////////////////////////////////////////////////////
// SPU_d_CLOSE: called before shutdown
////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////
// SPU_d_SHUTDOWN: called by main emu on final exit
////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////
// SPU_d_TEST: we don't test, we are always fine ;)
////////////////////////////////////////////////////////////////////////

long CALLBACK SPU_d_test(void)
{
 return 0;
}

////////////////////////////////////////////////////////////////////////
// SPU_d_CONFIGURE: call config dialog
////////////////////////////////////////////////////////////////////////

long CALLBACK SPU_d_configure(void)
{
#ifdef _WINDOWS
...
////////////////////////////////////////////////////////////////////////
// SPU_d_ABOUT: show about window
////////////////////////////////////////////////////////////////////////

void CALLBACK SPU_d_about(void)
{
#ifdef _WINDOWS
...
// passes a callback that should be called on SPU_d_-IRQ/cdda volume change
////////////////////////////////////////////////////////////////////////
