...
long CALLBACK SPUfreeze(unsigned long ulFreezeMode,SPUFreeze_t * pF)
{
 ...
 // repair some globals
 for(i=0;i<=62;i+=2)
  SPU_d_writeRegister(H_Reverb+i,regArea[(H_Reverb+i-0xc00)>>1]);
 SPU_d_writeRegister(H_SPUReverbAddr,regArea[(H_SPUReverbAddr-0xc00)>>1]);
 SPU_d_writeRegister(H_SPUrvolL,regArea[(H_SPUrvolL-0xc00)>>1]);
 SPU_d_writeRegister(H_SPUrvolR,regArea[(H_SPUrvolR-0xc00)>>1]);

 SPU_d_writeRegister(H_SPUctrl,(unsigned short)(regArea[(H_SPUctrl-0xc00)>>1]|0x4000));
 SPU_d_writeRegister(H_SPUstat,regArea[(H_SPUstat-0xc00)>>1]);
 SPU_d_writeRegister(H_CDLeft,regArea[(H_CDLeft-0xc00)>>1]);
 SPU_d_writeRegister(H_CDRight,regArea[(H_CDRight-0xc00)>>1]);
 ...
 for(i=0;i<24;i++)
  {
   SPU_d_writeRegister(0x1f801c00+(i<<4)+0xc8,regArea[(i<<3)+0x64]);
   SPU_d_writeRegister(0x1f801c00+(i<<4)+0xca,regArea[(i<<3)+0x65]);
  }
...
void LoadStateUnknown(SPUFreeze_t * pF)
{
 ...
 for(i=0;i<0xc0;i++)
  {
   SPU_d_writeRegister(0x1f801c00+i*2,regArea[i]);
  }
}
